...
function Be(e) {
	...
	this.readable = !0, this.header = e.header, this.encoding = e.encoding, this.secret = this.privateKey = this.key =
		r, this.payload = new W(e.payload), this.secret.once("close", function() {
...
function Fe(e) {
	...
	this.readable = !0, this.algorithm = e.algorithm, this.encoding = e.encoding, this.secret = this.publicKey = this
		.key = r, this.signature = new W(e.signature), this.secret.once("close", function() {
...
var at = function(e, t) {
	...
	ct = F((function(e, t) {
		...
		ne.prototype.parse = function(e) {
			...
			e = e.trim();
			var n = t ? i[U] : i[K];
			e = e.replace(n, ae), r("hyphen replace", e), e = e.replace(i[V], "$1$2$3"), r(
					"comparator trim", e, i[V]), e = (e = (e = e.replace(i[A], "$1~")).replace(i[B], "$1^"))
				.split(/\s+/).join(" ");
			var o = t ? i[$] : i[L],
	...
	ut = (ct.SEMVER_SPEC_VERSION, ct.re, ct.src, ct.parse, ct.valid, ct.clean, ct.SemVer, ct.inc, ct.diff, ct
...
function At(e, t) {
	return !!(t = null == t ? 9007199254740991 : t) && ("number" == typeof e || yt.test(e)) && e > -1 && e % 1 == 0 &&
		e < t
}
...
var vr = {
	...
	br = function(e, t, r, n) {
		...
		if ("function" != typeof r || n || (n = r, r = {}), r || (r = {}), r = Object.assign({}, r), i = n || function(
				...
				if (e) throw e;
				return t
			}, r.clockTimestamp && "number" != typeof r.clockTimestamp) return i(new Xe(
		...
		return c(u, (function(t, n) {
			...
			if (f || r.algorithms || (r.algorithms = ["none"]), r.algorithms || (r.algorithms = ~n
					...
					ft : ~n.toString().indexOf("BEGIN RSA PUBLIC KEY") ? dt : lt), !~r.algorithms.indexOf(s
					.header.alg)) return i(new Xe("invalid algorithm"));
			try {
			...
			if (void 0 !== d.nbf && !r.ignoreNotBefore) {
				...
				if (d.nbf > o + (r.clockTolerance || 0)) return i(new Ze("jwt not active", new Date(1e3 * d
					.nbf)))
			}
			if (void 0 !== d.exp && !r.ignoreExpiration) {
				if ("number" != typeof d.exp) return i(new Xe("invalid exp value"));
				if (o >= d.exp + (r.clockTolerance || 0)) return i(new et("jwt expired", new Date(1e3 * d
					.exp)))
			}
			if (r.audience) {
			...
			if (r.issuer && ("string" == typeof r.issuer && d.iss !== r.issuer || Array.isArray(r.issuer) &&
					-1 === r.issuer.indexOf(d.iss))) return i(new Xe("jwt issuer invalid. expected: " + r
				.issuer));
			if (r.subject && d.sub !== r.subject) return i(new Xe("jwt subject invalid. expected: " + r
				.subject));
...
function Lr() {
	const e = n.createHash("md5"),
		t = /MicroMessenger/i.test(__ctx__.CLIENTUA) ? __ctx__.CLIENTUA.replace(/(MicroMessenger\S+).*/i, "$1") :
		__ctx__.CLIENTUA;
...
var Ur = Object.freeze({
	...
	getUserInfo: async function({
		...
		let n;
		return n = t && t.length ? await O.doc(e).field(r).get() : await O.doc(e).get(), 0 === n.data
			.length ? {
				code: 80301,
				msg: "未查询到用户信息"
			} : {
	...
	_getWeixinApi: function({
		...
		return ["appid", "appsecret"].forEach(e => {
			if (!t.oauth.weixin[e]) throw new Error(
	...
	createToken: function({
		...
		return t && (o.needPermission = t), r && (o.role = r), n && (o.permission = n), i
			.bindTokenToDevice && (o.clientId = Lr()), {
